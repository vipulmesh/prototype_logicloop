...
interface StoredResume { fileName: string | null; }
...
export default function JobDetailsPage() {
  ...
  useEffect(() => { const found = getAvailableJobs().find((item) => item.id === params.id) || null; setJob(found); setApplied(getApplications().some((application) => application.jobId === params.id)); }, [params.id]);

  const apply = () => {
    if (!job) return;
    const resume = (() => { try { return JSON.parse(sessionStorage.getItem('talentai_resume') || 'null') as StoredResume | null; } catch { return null; } })();
    const report = getCachedAnalysis()?.report;
    if (!resume?.fileName || !report) { setError('Analyze a resume first, then return to apply with one click.'); return; }
    const application: JobApplication = { id: crypto.randomUUID(), jobId: job.id, jobTitle: job.title, candidateName: resume.fileName.replace(/\.[^.]+$/, ''), resumeName: resume.fileName, talentScore: report.overallScore, atsScore: report.atsScore, skills: report.technicalSkills, appliedAt: new Date().toISOString(), status: 'Applied' };
    saveApplications([application, ...getApplications()]); setApplied(true); setError(null);
  };
